...
class PostPublishHook(Hook):
    ...
    """    
    def execute(self, work_template, primary_task, secondary_tasks, progress_cb, sg_task, thumbnail_path, comment, **kwargs):
        """
    ...
    def _do_maya_post_publish(self, work_template, secondary_tasks, progress_cb, sg_task, thumbnail_path, comment):
        """
        Do any Maya post-publish work

        :param work_template:   The primary work template used for the publish
        :param secondary_tasks  The secondary publish tasks performed.
        ...
        # Before we up the version, use the fields to build our manifest
        # path in case we end up grouping.
        manifest_template = self.parent.get_template_by_name("publish_group_manifest")
        manifest_path = manifest_template.apply_fields(fields)
        ...
        # Group together all of the alembic_cache publishes if we have been
        # asked to do so in the config. <jbee>
        children = [st["item"]["sg_data"] for st in secondary_tasks if st["output"]["name"] == "alembic_cache"]
        secondary_outputs = [so for so in self.parent.settings["secondary_outputs"]]
        abc_config = [so for so in secondary_outputs if so["name"] == "alembic_cache" and so["publish_group"]]
        if children and abc_config:
            self._build_group_manifest(
                children=children,
                path=manifest_path,
            )
            self._register_publish(
                path=manifest_path,
                name=abc_config[0]["group_name"],
                sg_task=sg_task,
                ...
                dependency_paths=[],
                sg_fields=dict(
                    sg_children=children,
                ),
            )
        
        progress_cb(100)

    def _build_group_manifest(self, children, path):
        import json
        with open(path, 'w') as fh:
            json.dump(children, fh, indent=4)
    ...
    def _register_publish(
        self, path, name, sg_task, publish_version, tank_type, comment, thumbnail_path,
        dependency_paths, sg_fields=None
    ):
        """
        Helper method to register publish using the 
        specified publish info.
        """
        sg_fields = sg_fields or dict()
        # construct args:
        args = dict(
            tk=self.parent.tank,
